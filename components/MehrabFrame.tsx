...
export const MehrabFrame: React.FC<MehrabFrameProps> = ({ children, variant = 'primary', className = '' }) => {
  return (
    <div className={`relative p-4 md:p-12 ${className} group`}>
      ...
      <div
        className={`relative w-full h-full min-h-[550px] md:min-h-[600px] border-[4px] md:border-[6px] border-[#D4AF37] overflow-hidden bg-[#FFF8E7] shadow-[0_20px_60px_rgba(0,0,0,0.3)] transition-transform duration-700 rounded-[30px] md:rounded-[40px]`}
      >
        {/* Layered Inner Borders for Depth */}
        <div className="absolute inset-1.5 md:inset-2 border-[1px] border-[#D4AF37]/20 pointer-events-none rounded-[24px] md:rounded-[32px]"></div>
